---
title: "Basic Data Types and Syntax in R"
subtitle: "PSU R and PUMS Workshop"
author: "Jamaal Green"
date: 2018-07-29
output:
  tufte::tufte_handout:
...
This sheet is to provide some minimal introductory knowledge for brand new beginners and to get us acquainted with the idea of [*tidy*](http://r4ds.had.co.nz/explore-intro.html) data from Hadley Wickham. We will try not to get bogged down into too much unnecessary detail here but we want to introduce (briefly) some of the basic data types in R, assignment and calling functions. 
...
![Tidy Data](/day1/tidy-1.png)

Fig. 1- Tidy data diagram taken from [Wickham and Grolemund](http://r4ds.had.co.nz/introduction.html)


#R Projects and Working Directories

Before we start on a new analytical project we have to set a working directory. A working directory is a folder that R will look into when you attempt to import or export files. Traditionally, we can use the `setwd(<path to folder>)` to set a working directory or, in RStudio, click Session -> Set Working Directory -> Choose Directory. Setting a working directory allows you to use relative path names when calling for files as opposed to absolute paths. An easier way to keep track of your directories without having to include setwd() at the beginning of all of your scripts is to use R projects. R projects are a format developed at RStudio that create a self-contained project directory. So, for all files that exist within that project directory, you can reference them relative to the home folder. R projects make keep track of directories simple and give us a basic organizing piece for a data project. If you have a new project, create a new R project, move your files into it and work from there. Additionally, tanks to better integration of R with different version control applications, you can use R projects with git/GitHub, creating an easy way to share your work without having to worry about paths breaking down.